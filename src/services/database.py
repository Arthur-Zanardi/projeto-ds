...
from src.schema.schema_vetores import (
    ...
    flatten_storage_vectors,
    normalize_profile_vectors,
)


COLLECTION_NAME = f"perfis_matchai_{len(STORAGE_VECTOR_ORDER)}d"
...
def salvar_perfil_usuario(
    ...
    print(f"Perfil vetorial de {nome} salvo: {len(vetor_usuario)} dimensões.")
    return vetor_usuario
...
def popular_banco_mock() -> None:
    ...
    def physical(**updates: float) -> dict[str, float]:
        values = {key: 0.5 for key in PHYSICAL_VECTOR_SCHEMA.keys()}
    ...
    perfis = [
        ...
        (
            "user_luiza",
            "Luiza",
            "Música, astronomia, cabelo castanho, óculos e conversas profundas.",
            {
                "psicologico": {
                    "extroversao": 0.35,
        ...
        (
            "user_carmen",
            "Carmen",
            "Caseira, religiosa, elegante, cabelo preto e apaixonada por culinária.",
            {
                "psicologico": {
                    "extroversao": 0.25,
